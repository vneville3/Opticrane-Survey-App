// Opticrane Survey App — Standard-only v1.3.2
const VERSION = '1.3.2';
...
// Photo buffer (append, not replace)
let photoBuffer = [];
...
function updatePreview(){
  const p=$('#preview'); const count=$('#photoCount'); p.innerHTML='';
  photoBuffer.forEach((ph,idx)=>{
    const wrap=document.createElement('div'); wrap.className='thumb';
    const img=document.createElement('img'); img.src=ph.dataUrl; wrap.appendChild(img);
    const rm=document.createElement('button'); rm.type='button'; rm.textContent='×'; rm.onclick=()=>{ photoBuffer.splice(idx,1); updatePreview(); };
...
// Share with attachments (auto-copy To)
...
async function shareLastEntry(){
  ...
  const files = [ jsonToFile(last, `${safe}.json`) ];
  (last.photos||[]).forEach((p,idx)=>files.push(dataUrlToFile(p.dataUrl, p.filename || `photo_${idx+1}.jpg`)));

  // Copy the To address so you can paste in Mail composer
  const toAddr = (state.settings.collectionEmail||'').trim();
  if (toAddr && navigator.clipboard && navigator.clipboard.writeText) {
    try { await navigator.clipboard.writeText(toAddr); toast('To address copied — paste into Mail.'); } catch {}
  }

  if (navigator.canShare && navigator.canShare({ files })) {
    try{
      await navigator.share({ files, title: subj, text: `To: ${toAddr || 'you@opticrane.com'}` });
      // iOS Mail uses the first attachment filename as subject; now user pastes To.
    }catch(e){ if(e.name!=='AbortError'){ alert('Share failed: '+e.message); } }
  } else {
    composeEmailNoAttachments(); // fallback
  }
}

// Mailto (no attachments) with To + Subject prefilled
function composeEmailNoAttachments(){
  const last = state.queue[state.queue.length-1]?.data;
  const to = encodeURIComponent((state.settings.collectionEmail||'').trim());
  const subj = encodeURIComponent(`${state.settings.subjectPrefix || 'Opticrane Survey'} — ${last?.contactName || 'Contact'}`);
...
function init(){
  ...
  $('#composeMailto').addEventListener('click', composeEmailNoAttachments);
  $('#exportJson').addEventListener('click', ()=>{ const blob=new Blob([JSON.stringify(state.queue,null,2)],{type:'application/json'}); const url=URL.createObjectURL(blob); const a=document.createElement('a'); a.href=url; a.download='opticrane_queue.json'; a.click(); URL.revokeObjectURL(url); });
  $('#exportCsv').addEventListener('click', ()=>{ const rows=[['timestamp','contactName','role','siteCity','environment','machine','targetView','impactFreq','timeLost','timeline','reason','reasonNote','techInitials','photoCount']]; for(const it of state.queue){const d=it.data||{}; rows.push([d.timestamp,d.contactName,d.role,d.siteCity,d.environment,d.machine,d.targetView,d.impactFreq,d.timeLost,d.timeline,d.reason,d.reasonNote,d.techInitials,(d.photos||[]).length]);} const csv=rows.map(arr=>arr.map(v=>`"${(v??'').toString().replace(/"/g,'""')}"`).join(',')).join('\n'); const blob=new Blob([csv],{type:'text/csv'}); const url=URL.createObjectURL(blob); const a=document.createElement('a'); a.href=url; a.download='opticrane_queue.csv'; a.click(); URL.revokeObjectURL(url); });
  $('#clearAll').addEventListener('click', ()=>{ if(!confirm('Erase drafts and the local queue on this device?')) return; state.queue=[]; state.draft={}; photoBuffer=[]; localStorage.removeItem(LS.queue); localStorage.removeItem(LS.draft); updatePreview(); setStatus(''); toast('Local data cleared.'); });
